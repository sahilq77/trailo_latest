...
class PickedByOutwardScreen extends StatefulWidget {
  const PickedByOutwardScreen({super.key});

  @override
  _PickedByOutwardScreenState createState() => _PickedByOutwardScreenState();
}

class _PickedByOutwardScreenState extends State<PickedByOutwardScreen> {
  final PickedByListController controller = Get.put(PickedByListController());
  ...
  void initState() {
    super.initState();
    controller.fetchPickedByList(context: context);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Picked By Outward List'),
          actions: [
            IconButton(
        ...
        body: RefreshIndicator(
          ...
          child: Obx(() {
            ...
            if (controller.pickedByList.isEmpty &&
                !controller.isLoadingMore.value) {
            ...
            return ListView.builder(
              ...
              itemCount:
                  controller.pickedByList.length +
                  (controller.hasMoreData.value ||
                          controller.isLoadingMore.value
                      ? 1
                      : 0),
              itemBuilder: (context, int index) {
                if (index == controller.pickedByList.length) {
                ...
                var outward = controller.pickedByList[index];
                return GestureDetector(
                  onTap: () {
                    Get.toNamed(
                      AppRoutes.pickedbyoutwarddetail,
                      arguments: {
                        "picked_by_detail": outward,
                        "sr_no": "${index + 1}",
                      },
                    );
                  },
                  child: Card(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        ...
                        border: Border(
                          left: BorderSide(
                            color: _getStatusColor(outward.status.toString()),
                            width: 5,
                          ),
                        ),
                      ),
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildDetailRow(
                              'Company',
                              outward.companyName.toString(),
                            ),
                            const SizedBox(height: 6),
                            _buildDetailRow(
                              'Division',
                              outward.divisionName.toString(),
                            ),
                            const SizedBox(height: 6),
                            _buildDetailRow(
                              'Date',
