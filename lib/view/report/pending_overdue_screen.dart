...
class PendingOverdueScreen extends StatefulWidget {
  const PendingOverdueScreen({super.key});

  @override
  _PendingOverdueScreenState createState() => _PendingOverdueScreenState();
}

class _PendingOverdueScreenState extends State<PendingOverdueScreen> {
  final PendingOverdueListController controller = Get.put(
    PendingOverdueListController(),
  );
  ...
  void initState() {
    super.initState();
    controller.fetchOverdue(context: context);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Pending Overdue List'),
        actions: [
          IconButton(
      ...
      body: RefreshIndicator(
        ...
        child: Obx(() {
          ...
          if (controller.pendingOverdueList.isEmpty &&
              !controller.isLoadingMore.value) {
          ...
          return ListView.builder(
            ...
            itemCount:
                controller.pendingOverdueList.length +
                (controller.hasMoreData.value || controller.isLoadingMore.value
                    ? 1
                    : 0),
            itemBuilder: (context, int index) {
              if (index == controller.pendingOverdueList.length) {
              ...
              var outward = controller.pendingOverdueList[index];
              return GestureDetector(
                onTap: () {
                  Get.toNamed(
                    AppRoutes.overdueDetails,
                    arguments: {
                      "overdue_detail": outward,
                      "sr_no": "${index + 1}",
                    },
                  );
                },
