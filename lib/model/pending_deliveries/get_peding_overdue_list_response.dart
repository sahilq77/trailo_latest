...
List<GetPendingOverdueListReponse> getPendingOverdueListListResponseFromJson(
  String str,
) {
  try {
    final decoded = json.decode(str);
    if (decoded is List) {
      return List<GetPendingOverdueListReponse>.from(
        decoded.map(
          (x) =>
              GetPendingOverdueListReponse.fromJson(x as Map<String, dynamic>),
        ),
      );
    }
    return [];
...
String getPendingOverdueListResponseToJson(
  List<GetPendingOverdueListReponse> data,
) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));

class GetPendingOverdueListReponse {
  String status;
  String message;
  String documentPath;
  List<PendingOverdueData> data;

  GetPendingOverdueListReponse({
  ...
  factory GetPendingOverdueListReponse.fromJson(Map<String, dynamic> json) {
    return GetPendingOverdueListReponse(
      status: json['status'] as String? ?? '',
      message: json['message'] as String? ?? '',
      documentPath: json['document_path'] as String? ?? '',
      data: json['data'] is List
          ? List<PendingOverdueData>.from(
              (json['data'] as List).map(
                (x) => PendingOverdueData.fromJson(x as Map<String, dynamic>),
              ),
            )
          : [],
    );
