...
class CheckedListController extends GetxController {
  var checkedByList = <CheckedByListData>[].obs;
  ...
  void onInit() {
    super.onInit();
    fetchCheckedList(context: Get.context!);
  }

  Future<void> fetchCheckedList({
    ...
    try {
      ...
      List<GetCheckedByListResponse>? response =
          (await Networkcall().postMethod(
                Networkutility.checkedByListApi,
                Networkutility.checkedByList,
                jsonEncode(jsonBody),
                context,
              ))
              as List<GetCheckedByListResponse>?;
      ...
      if (response != null && response.isNotEmpty) {
        if (response[0].status == "true") {
          ...
          final existingIds = checkedByList.map((item) => item.id).toSet();
          ...
          offset.value += limit;
          log(
            'Offset updated to: ${offset.value}, Total items: ${checkedByList.length}',
          );
        } else {
          hasMoreData.value = false;
    ...
    try {
      ...
      if (list != null && list.isNotEmpty) {
        ...
        if (response[0].status == "true") {
          ...
          await fetchCheckedList(context: context!, reset: true);
  ...
  Future<void> setPacked({
    ...
    try {
      ...
      List<Object?>? list = await Networkcall().postMethod(
        Networkutility.transferPackedApi,
        Networkutility.transferPacked,
        jsonEncode(jsonBody),
        Get.context!,
      );

      if (list != null && list.isNotEmpty) {
        List<GetTransferPackedResponse> response = List.from(list);

        if (response[0].status == "true") {
          Get.snackbar(
            'Success',
            'Transfer to Packed Successfully',
            backgroundColor: AppColors.success,
            colorText: Colors.white,
            duration: Duration(seconds: 3),
          );
